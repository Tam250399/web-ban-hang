...
//
// Trước đây giao diện dùng emoji làm icon. Emoji tiện lúc viết nhưng có ba
// nhược điểm không sửa được bằng CSS: mỗi hệ điều hành vẽ một kiểu (🏗️ trên
// Windows khác hẳn trên Android), chúng luôn tự tô màu nên không đổi theo
// `currentColor` khi nút đổi trạng thái, và chiều cao dòng của chúng không
// khớp với chữ nên hay bị lệch một hai pixel so với nhãn bên cạnh.
//
// Toàn bộ icon ở đây vẽ theo lưới 24×24, nét 2px, dùng `currentColor` — đặt
// trong nút màu gì thì icon theo màu đó.
//
// Mặc định `aria-hidden`: icon luôn đi kèm nhãn chữ hoặc nằm trong nút đã có
// `aria-label`, nên để máy đọc màn hình đọc thêm lần nữa chỉ gây nhiễu. Trường
// hợp icon đứng một mình và MANG thông tin thì truyền `title` để nó tự mô tả.
// ─────────────────────────────────────────────────────────────────────────
...
export function Icon({ name, size = 18, title, className = '', style }) {
  const path = P[name]
  // Tên sai thì không dựng khoảng trắng lơ lửng giữa nhãn — thà không có icon.
  if (!path) return null
